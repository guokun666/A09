...
public class UserAgeController {
    ...
    public Object getSignalLineUserAge(@RequestParam("year") Integer year,
        ...
        BigInteger youth=BigInteger.ZERO;
        BigInteger midYouth=BigInteger.ZERO;
        BigInteger middleAge=BigInteger.ZERO;
        BigInteger old=BigInteger.ZERO;
        //获取table信息
        for(UserAgeEntity station:userAgeService.getStationUserAgeByYearAndLine(year, line)){
            youth.add(station.getYouth());
            midYouth.add(station.getMidYouth());
            middleAge.add(station.getMiddleAge());
            old.add(station.getOld());
        }
        ...
        tableJson.put("proportion",nf.format(youth.doubleValue()/all));
